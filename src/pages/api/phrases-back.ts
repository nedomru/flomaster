...
const phrasesBack: StructuredData = {
    "Технические вопросы": {
        subcategories: {
            "Интернет": {
                phrases: {
...
Если возникнут сложности или дополнительные вопросы - обращайтесь! Будем рады помочь 😉`
            ...
            "Быстрый чат": {
                phrases: {
                    "Авария (с компенсацией) - сроки есть": {
                        default: {
                            value: `К сожалению, по вашему адресу зафиксированы временные ограничения в предоставлении услуг.
...
В качестве компенсации сделали перерасчет за сутки`
                    ...
                    "Авария (с компенсацией) - сроков нет": {
                        default: {
                            value: `К сожалению, по вашему адресу зафиксированы временные ограничения в предоставлении услуг.
Технические специалисты уже направлены для устранения неполадок.
...
В качестве компенсации сделали перерасчет за сутки`
                    ...
                    "ППР (без компенсации) - сроки есть": {
                        default: {
                            value: `По Вашему адресу проводятся планово-профилактические работы.
Прогноз завершения: *дата и время*

От лица Компании приносим вам извинения за доставленные неудобства🙏🏼`
                        }
                    },
                    "ППР (без компенсации) - сроков нет": {
                        default: {
                            value: `По Вашему адресу проводятся планово-профилактические работы.
...
Если будут вопросы или сложность повторится, пожалуйста, напишите нам.`
                    ...
                    "Отключение услуг по ДЗ": {
                        default: {
                            value: `Добрый день! Сумма задолженности на *число* составляет *сумма*
...
Оплатить Вы можете любым удобным для себя способом: через ЛК, на сайте или через мобильное приложение банка.`
                    ...
                    "Обслуживание домофонии Цифрал-Сервис": {
                        default: {
                            value: `Обслуживанием домофона и мобильного приложения занимается "Цифрал-Сервис"🛠
...
export const GET: APIRoute = ({request}) => {
    ...
    if (categoryName && phrasesBack[categoryName]) {
        if (subCategoryName && phrasesBack[categoryName].subcategories[subCategoryName]) {
            Object.keys(phrasesBack[categoryName].subcategories[subCategoryName].phrases).forEach(phrase => {
                filterAndAddToResult(phrasesBack, categoryName, subCategoryName, phrase);
            });
        } else {
            Object.keys(phrasesBack[categoryName].subcategories).forEach(subcategory => {
                Object.keys(phrasesBack[categoryName].subcategories[subcategory].phrases).forEach(phrase => {
                    filterAndAddToResult(phrasesBack, categoryName, subcategory, phrase);
                });
            });
        }
    } else {
        Object.keys(phrasesBack).forEach(category => {
            Object.keys(phrasesBack[category].subcategories).forEach(subcategory => {
                Object.keys(phrasesBack[category].subcategories[subcategory].phrases).forEach(phrase => {
                    filterAndAddToResult(phrasesBack, category, subcategory, phrase);
                });
            });
        });
    }
